...
def load_aligned(paths, labels=None):
    """Load several per-question .jsonls and pair them BY QUESTION TEXT.

    Pairing the per-question outcomes is only valid when row i refers to the SAME
    question in every file — otherwise the paired bootstrap and McNemar test
    silently pair unrelated questions and report a corrupt difference WITHOUT
    erroring (they index purely by position). Files written by different runs can
    share the same test split but in a different row order (e.g. the eval data
    order is shuffled by SEED), so guarding on positional equality alone is both
    too strict (rejects a mere reordering) and the only thing standing between a
    reorder and silent corruption.

    This therefore joins on the question text instead of trusting position:
      * every file must contain the SAME SET of questions (same count, no
        duplicates, identical set) — otherwise the models weren't evaluated on
        the same split and a comparison is meaningless;
      * each file's rows are then reindexed into one canonical question order
        (the first file's order), so the returned columns are guaranteed paired
        regardless of the on-disk order.

    Returns (labels, cols_list, n) with every cols dict reordered to the canonical
    order.
    """
    import numpy as np
    if len(paths) < 2:
    ...
    loaded = [(path, *load_metrics(path)) for path in paths]  # (path, cols, questions, n)
    ref_path, _, ref_q, n = loaded[0]
    if len(set(ref_q)) != n:
        raise ValueError(
            f"{ref_path!r} contains duplicate questions; cannot join on question "
            f"text (a question maps to more than one row).")
    ref_set = set(ref_q)
    canonical_index = {q: i for i, q in enumerate(ref_q)}

    cols_list = []
    for path, cols, questions, count in loaded:
        if count != n:
            raise ValueError(
                f"{path!r} has {count} questions but {ref_path!r} has {n}; the "
                f"files must cover the same test split to be paired.")
        if len(set(questions)) != count:
            raise ValueError(f"{path!r} contains duplicate questions; cannot join.")
        if set(questions) != ref_set:
            missing = ref_set - set(questions)
            extra = set(questions) - ref_set
            raise ValueError(
                f"{path!r} covers a different question set than {ref_path!r} "
                f"({len(missing)} missing, {len(extra)} extra); the files do not "
                f"cover the same test split and cannot be paired.")
        # Reindex this file's rows into the canonical question order.
        order = np.empty(n, dtype=np.int64)
        for src_row, q in enumerate(questions):
            order[canonical_index[q]] = src_row
        cols_list.append({m: cols[m][order] for m in METRICS})
